...
@code-desc: Distributed Min-Max Transformation using Tree-based Broadcasting with Core Affinity
*/
...
int connect_to_slave(const char *ip, int port) {
    ...
    int retries = 300; // 300 retries * 100ms = 30 seconds
    while (retries > 0) {
...
void send_matrix_data(int socket, double **matrix, int rows, int cols, int start_row) {
    int total_sent = 0;
    int bytes_to_send = rows * cols * sizeof(double);
    char *data_ptr = (char *)matrix[start_row];
    
    while (total_sent < bytes_to_send) {
        int sent = send(socket, data_ptr + total_sent, 
...
void receive_matrix_data(int socket, double **matrix, int rows, int cols) {
    ...
    int bytes_to_receive = rows * cols * sizeof(double);
    char *data_ptr = (char *)matrix[0];
    
    while (total_received < bytes_to_receive) {
        int received = recv(socket, data_ptr + total_received, 
...
double ** generate_matrix(int row, int col){
    ...
    double *data_block = (double *)malloc(sizeof(double) * row * col);

    for(int i = 0; i < row; i++){
        temp[i] = &data_block[i * col];
    }
    return temp;
}
