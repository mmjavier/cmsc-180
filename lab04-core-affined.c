...
int main(int argc, char **argv) {
    ...
    if (s == 0) {
        ...
        // Apply Core Affinity for the Slave Process
#ifdef __linux__
        int core_count = (int)sysconf(_SC_NPROCESSORS_ONLN);
        // Reserve 1 core for the OS/Master if we have multiple cores available
        int available_cores = (core_count > 1) ? (core_count - 1) : 1;
        int cpu_id = rank % available_cores;
        
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
        CPU_SET(cpu_id, &cpuset);

        int aff_status = sched_setaffinity(0, sizeof(cpu_set_t), &cpuset);
        if (aff_status != 0) {
            fprintf(stderr, "Slave %d: sched_setaffinity failed for core %d: %s\n",
                    rank, cpu_id, strerror(errno));
        } else {
