...
@code-desc: Implementing Min-Max Transformation on an nxn matrix in C using Threads (row-major via transpose)
*/
...
typedef struct {
    ...
    int cpu_id;          // Target CPU core for affinity
} args;
...
int main(int argc, char **argv) {
    ...
    //Transpose first so row-wise normalization becomes column-wise on the transposed matrix
    ...
    for (int i = 0; i < t; i++) {
        ...
        arguments[i].cpu_id = i % core_count;
